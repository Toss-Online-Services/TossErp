    ...
    if (Test-Path "package.json") {
        if (-not $SkipBuild) {
            ...
            if ($LASTEXITCODE -ne 0) {
                Write-Warning "npm install failed, trying npm ci..."
                npm ci
            }
...
try {
    ...
    if ($LASTEXITCODE -eq 0) {
        ...
        if ($runningContainers) {
            ...
            Write-Warning "No TOSS ERP containers are currently running"
...
Write-Info ""
Write-Info "1. Complete Environment (Recommended):"
Write-Info "   Run: .\scripts\start-local-env.bat"
Write-Info "   Access: http://localhost:3001"
Write-Info ""
Write-Info "2. Web Client Only:"
Write-Info "   Run: .\scripts\start-web-client.bat"
Write-Info "   Note: Requires backend services running separately"
Write-Info ""
Write-Info "3. Infrastructure Only:"
...
Write-Info "   Database Admin:    http://localhost:5050"
...
Write-Info ""
Write-Warning "Note: Ensure Docker Desktop is running before starting services"
Write-Info ""
