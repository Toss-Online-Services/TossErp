      ...
      if (unref(isVisible)) {
        ...
        if (unref(isOpen)) {
          ...
          ssrRenderList(unref(messages), (message) => {
            ...
            if (message.type === "bot") {
              _push(`<div class="flex items-start space-x-2"><div class="w-6 h-6 bg-green-500 rounded-full flex items-center justify-center flex-shrink-0"><svg class="w-3 h-3 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9.663 17h4.673M12 3v1m6.364 1.636l-.707.707M21 12h-1M4 12H3m3.343-5.657l-.707-.707m2.828 9.9a5 5 0 117.072 0l-.548.547A3.374 3.374 0 0014 18.469V19a2 2 0 11-4 0v-.531c0-.895-.356-1.754-.988-2.386l-.548-.547z"></path></svg></div><div class="bg-gray-100 dark:bg-gray-700 rounded-lg p-3 max-w-xs"><p class="text-sm text-gray-900 dark:text-white">${(_a = message.content) != null ? _a : ""}</p><p class="text-xs text-gray-500 dark:text-gray-400 mt-1">${ssrInterpolate(formatTime(message.timestamp))}</p></div></div>`);
            } else {
...
//# sourceMappingURL=AICopilotChat-BbZcaCTr.mjs.map
