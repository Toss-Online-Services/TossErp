    ...
    <div class="relative">
      ...
      <div class="space-y-6">
        ...
        <div class="relative flex items-start gap-4">
          <div 
            class="flex-shrink-0 w-8 h-8 rounded-full flex items-center justify-center z-10 ring-4 ring-white dark:ring-slate-800"
            :class="isInTransit ? 'bg-orange-500' : 'bg-slate-300 dark:bg-slate-600'"
          >
          ...
          <div class="flex-1 pb-6">
            <div class="bg-white dark:bg-slate-800 rounded-lg p-4 shadow-sm border border-slate-200 dark:border-slate-700">
              <div class="flex items-center justify-between mb-2">
                <h4 class="font-bold text-slate-900 dark:text-white">Out for Delivery</h4>
                <span v-if="isInTransit" class="text-xs text-slate-500 dark:text-slate-400">
                  {{ formatDate(inTransitDate) }}
                </span>
              </div>
              <p class="text-sm text-slate-600 dark:text-slate-400">
                <span v-if="isInTransit">Your order is on the way to your location.</span>
                <span v-else class="italic">Not dispatched yet...</span>
              </p>
              <div v-if="isInTransit && estimatedDelivery" class="mt-2 flex items-center gap-2 text-sm text-green-600 dark:text-green-400">
                <ClockIcon class="w-4 h-4" />
                <span>Estimated arrival: {{ formatDate(estimatedDelivery) }}</span>
              </div>
            </div>
          </div>
        </div>
...
// Compute timeline progress based on status
const isConfirmed = computed(() => {
  return ['approved', 'in-transit', 'delivered'].includes(props.status.toLowerCase())
})

const isPreparing = computed(() => {
  return ['in-transit', 'delivered'].includes(props.status.toLowerCase())
})
...
const inTransitDate = computed(() => {
  if (!isInTransit.value) return null
  const date = new Date(props.orderDate)
