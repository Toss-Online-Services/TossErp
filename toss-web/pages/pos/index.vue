...
function handleSendReceipt() {
  if (!posStore.currentSale || !posStore.currentSale.customerName) {
    return
  }
  
  // TODO: Implement actual send functionality (email/SMS)
  // For now, show a success message
  alert(`Receipt will be sent to ${posStore.currentSale.customerName}`)
  
  // In production, this would:
  // 1. Get customer email/phone from customerId
  // 2. Generate PDF or HTML receipt
  // 3. Send via email or SMS service
...
<template>
  <div class="min-h-screen bg-gray-100 pb-12">
    ...
    <div class="sticky top-2 z-30 mx-3 mt-2 rounded-lg bg-white/80 backdrop-blur-md shadow-md px-4 py-3 mb-4">
      <div class="flex items-center justify-between">
        <div class="flex items-center gap-4">
          ...
          <div v-if="pendingCount > 0" class="flex items-center gap-2 px-3 py-1 bg-gray-100 text-gray-900 rounded-full text-sm">
        ...
        <div class="flex items-center gap-2">
          <button
            ...
            :class="{ 'bg-gray-100 text-gray-900': selectedCustomer }"
          >
    ...
    <div class="px-4">
      <div class="grid grid-cols-1 lg:grid-cols-12 gap-6">
          ...
          <div class="bg-white rounded-lg shadow-sm mb-6">
            ...
            <div class="flex flex-col md:flex-row gap-4 mb-4">
              <div class="flex-1 relative">
                <input
                  ...
                  class="w-full px-4 py-3 pr-12 border border-gray-300 rounded-lg focus:outline-none focus:border-gray-900 focus:ring-2 focus:ring-gray-900/20 text-base"
                  autofocus
                >
                <i class="material-symbols-rounded absolute right-4 top-1/2 -translate-y-1/2 text-gray-400">search</i>
              </div>
              <button class="inline-flex items-center justify-center gap-2 px-4 py-3 text-sm font-bold text-white uppercase rounded-lg shadow-sm hover:shadow-md transition-all bg-gradient-to-br from-gray-800 to-gray-900">
            ...
            <div class="flex gap-2 overflow-x-auto pb-2 scrollbar-hide">
              <button
                ...
                :class="[
                  'px-4 py-2 mb-0 text-xs font-bold uppercase rounded-lg whitespace-nowrap transition-all duration-200 ease-in-out',
                  selectedCategory === cat
                    ? 'bg-gradient-to-br from-gray-800 to-gray-900 text-white shadow-sm hover:shadow-md active:shadow-sm active:scale-[0.98]'
                    : 'bg-white text-gray-900 border border-gray-900 hover:bg-gray-50 hover:border-gray-800 hover:shadow-sm active:bg-gray-100 active:scale-[0.98]'
                ]"
              >
          ...
          <div class="bg-white rounded-lg shadow-sm">
            ...
            <div v-else class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-3 xl:grid-cols-4 gap-4">
              <div
                ...
                :key="item.id"
                class="bg-white rounded-lg shadow-sm hover:shadow-md transition-all cursor-pointer p-3 border border-gray-200"
                @click="addItem(item)"
              >
                <div class="aspect-square bg-gradient-to-br from-gray-800 to-gray-900 rounded-lg mb-3 flex items-center justify-center">
                  <i class="material-symbols-rounded text-4xl text-white opacity-10">inventory_2</i>
                </div>
                <h6 class="font-bold text-gray-900 mb-1 line-clamp-2 text-sm">{{ item.name }}</h6>
                <p class="text-xs text-gray-600 mb-2">{{ item.code }}</p>
                <div class="flex items-center justify-between mt-2">
                  <span class="text-sm font-bold text-gray-900">{{ formatCurrency(item.sellingPrice) }}</span>
                  <span class="text-xs bg-gray-200 text-gray-700 px-2 py-1 rounded">{{ item.currentStock }} {{ item.unit }}</span>
                </div>
              </div>
            </div>
            </div>
        ...
        <div class="hidden lg:block lg:col-span-4">
          <div class="bg-white rounded-lg shadow-sm flex flex-col sticky" style="top: 6rem; max-height: calc(100vh - 8rem);">
            <!-- Cart Header -->
            <div class="bg-gradient-to-br from-gray-800 to-gray-900 text-white rounded-t-lg">
              <div class="p-4">
              ...
              <div v-if="selectedCustomer" class="mt-2 text-xs text-white opacity-8">
            ...
            <div class="flex-1 overflow-y-auto p-4">
              ...
              <div v-else-if="posStore.cart && posStore.cart.length > 0" class="space-y-3">
                <div
                  ...
                  :key="item.id"
                  class="bg-gray-50 rounded-lg shadow-sm p-3 border border-gray-200"
                >
                  <div class="flex items-start justify-between mb-3">
                    <div class="flex-1">
                      <h3 class="font-semibold text-gray-900 text-sm mb-1">{{ item.name }}</h3>
                      <p class="text-xs text-gray-600">{{ item.code }}</p>
                      <p class="text-sm font-bold text-gray-900 mt-1">{{ formatCurrency(item.price) }} each</p>
                    </div>
                    <button
                  ...
                  <div class="flex items-center justify-between pt-2 border-t border-gray-200">
                    <button
                      ...
                      class="text-xs text-gray-900 hover:text-gray-800 flex items-center gap-1"
                    >
            ...
            <!-- Cart Summary -->
            <div class="border-t pt-4 pb-4 px-4 bg-gray-50 rounded-b-lg">
              <div class="space-y-3">
              ...
              <button
                @click="openPayment"
                :disabled="!posStore.cart || posStore.cart.length === 0"
                class="w-full py-3 text-sm font-bold text-white uppercase rounded-lg shadow-sm hover:shadow-md transition-all inline-flex items-center justify-center gap-2 bg-gradient-to-br from-gray-800 to-gray-900 disabled:opacity-50 disabled:cursor-not-allowed"
              >
    ...
    >
      <div class="bg-white rounded-lg max-w-md w-full max-h-[90vh] overflow-y-auto shadow-xl">
        <div class="p-6 border-b">
          ...
          <div class="bg-gradient-to-br from-gray-800 to-gray-900 rounded-lg p-4 text-white mb-4 shadow-sm">
        ...
        <div class="p-6 space-y-6">
          ...
          <div>
            ...
            <div class="grid grid-cols-3 gap-3">
              ...
              >
                <i class="material-symbols-rounded text-2xl mb-2 block" :class="paymentMethod === 'cash' ? 'text-gray-900' : 'text-gray-400'">payments</i>
                <p class="text-xs font-bold" :class="paymentMethod === 'cash' ? 'text-gray-900' : 'text-gray-600'">Cash</p>
              </button>
              <button
              ...
              >
                <i class="material-symbols-rounded text-2xl mb-2 block" :class="paymentMethod === 'card' ? 'text-gray-900' : 'text-gray-400'">credit_card</i>
                <p class="text-xs font-bold" :class="paymentMethod === 'card' ? 'text-gray-900' : 'text-gray-600'">Card</p>
              </button>
              <button
              ...
              >
                <i class="material-symbols-rounded text-2xl mb-2 block" :class="paymentMethod === 'mobile' ? 'text-gray-900' : 'text-gray-400'">phone_android</i>
                <p class="text-xs font-bold" :class="paymentMethod === 'mobile' ? 'text-gray-900' : 'text-gray-600'">Mobile</p>
              </button>
            </div>
          </div>
          ...
          <div v-if="paymentMethod === 'cash'">
            ...
            <input
              ...
              type="number"
              step="0.01"
              class="w-full px-4 py-3 text-2xl font-bold border border-gray-300 rounded-lg focus:outline-none focus:border-gray-900 focus:ring-2 focus:ring-gray-900/20"
              autofocus
            >
          </div>
          ...
          <!-- Change Display -->
          <div v-if="changeAmount > 0" class="bg-green-50 rounded-lg p-4 border border-green-200 shadow-sm">
          ...
          <div v-if="totalPaid < posStore.cartTotal && totalPaid > 0" class="bg-red-50 rounded-lg p-4 border border-red-200 shadow-sm">
            <div class="flex items-center gap-2">
          ...
          <button
            ...
            class="w-full py-3 text-sm font-bold text-white uppercase rounded-lg shadow-sm hover:shadow-md transition-all disabled:opacity-50 disabled:cursor-not-allowed inline-flex items-center justify-center gap-2 bg-gradient-to-br from-green-500 to-green-600"
          >
        ...
        <div class="max-h-96 overflow-y-auto p-4">
          <div class="space-y-2">
            <button
              ...
              class="w-full p-3 text-left rounded-lg hover:bg-gray-100 transition-all shadow-sm hover:shadow-md border border-gray-200 bg-white"
            >
              <p class="font-semibold text-gray-900 mb-0">{{ customer.name }}</p>
              <p class="text-sm text-gray-600 mb-0">{{ customer.phone }}</p>
            </button>
          </div>
          <button
            v-if="selectedCustomer"
            @click="clearCustomer"
            class="w-full mt-4 p-3 text-red-600 hover:bg-red-50 rounded-lg transition-all shadow-sm hover:shadow-md border border-red-200 bg-white font-semibold"
          >
            Clear Selection
          </button>
        </div>
      ...
      <div class="bg-white rounded-lg max-w-sm w-full shadow-xl">
        ...
        <div class="p-6">
          <input
            ...
            class="w-full px-4 py-3 text-lg border border-gray-300 rounded-lg focus:outline-none focus:border-gray-900 mb-4"
            autofocus
          >
          <div class="flex gap-2">
            <button
              @click="applyDiscount(discountAmount || 0)"
              class="flex-1 py-3 text-sm font-bold text-white uppercase rounded-lg shadow-sm hover:shadow-md transition-all bg-gradient-to-br from-gray-800 to-gray-900"
            >
              Apply
            </button>
            <button
              @click="showDiscountModal = false"
              class="flex-1 py-3 text-sm font-bold text-gray-700 uppercase rounded-lg shadow-sm hover:shadow-md transition-all bg-white border border-gray-300 hover:bg-gray-50"
            >
              Cancel
            </button>
          </div>
