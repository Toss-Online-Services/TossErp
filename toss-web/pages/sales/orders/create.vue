    ...
    <div class="flex items-center justify-between mb-6">
      <div>
        ...
        <p class="text-sm text-gray-500 dark:text-gray-400">
          {{ $t('sales.orders.create.description') }}
        </p>
      </div>
    </div>
      ...
      <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div class="lg:col-span-2 bg-white dark:bg-gray-800 shadow-sm rounded-lg p-6">
          <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
            <FormKit
              type="select"
              name="customerId"
              :label="$t('sales.orders.create.customer')"
              :placeholder="$t('sales.orders.create.selectCustomer')"
              :options="customers"
              validation="required"
            />
            <FormKit
              type="date"
              name="orderDate"
              :label="$t('sales.orders.create.orderDate')"
              validation="required"
            ...
            <FormKit
              type="select"
              name="quotationId"
              :label="$t('sales.orders.create.relatedQuotation')"
              :placeholder="$t('sales.orders.create.selectQuotation')"
              :options="['QT-001', 'QT-002']"
            />
            <FormKit
              type="select"
              name="status"
          ...
          <div class="mt-8">
            ...
            <FormKit type="list" name="items" :value="[{}]" dynamic #default="{ items, node, value: listValue }">
              <div v-for="(item, index) in items" :key="item" class="flex items-end gap-4 mb-4 p-4 border rounded-lg dark:border-gray-700">
                <div class="grid grid-cols-1 sm:grid-cols-5 gap-4 flex-grow">
                  <FormKit
                    ...
                    :placeholder="$t('sales.orders.create.selectProduct')"
                    :options="products"
                  ...
                  <div class="flex items-center sm:col-span-1">
                    <p class="text-sm font-medium text-gray-700 dark:text-gray-300 mt-6">
                      {{ formatCurrency(getLineTotal(listValue[index])) }}
                    </p>
                  </div>
                </div>
                <button type="button" @click="() => node.input(listValue.filter((_, i) => i !== index))" class="text-red-500 hover:text-red-700 mb-2">
        ...
        <div class="lg:col-span-1">
          <div class="bg-white dark:bg-gray-800 shadow-sm rounded-lg p-6 sticky top-6">
            ...
            <div class="mt-6">
              <FormKit type="submit" :label="$t('sales.orders.create.createButton')" outer-class="!mb-0" />
            </div>
          </div>
        </div>
      </div>
...
const { t } = useI18n()
useHead({
  title: t('sales.orders.create.pageTitle'),
})
...
const createOrder = async (formData: any) => {
  ...
  const payload = { ...formData, totals: totals.value }
  alert(`${t('sales.orders.create.successMessage')}!\n\n${JSON.stringify(payload, null, 2)}`)
}
