  ...
  <div class="p-4 sm:p-6">
    <div class="flex items-center justify-between mb-6">
      <h1 class="text-2xl font-bold text-gray-900 dark:text-white">
        {{ $t('sales.orders.edit.title') }} #{{ orderId }}
      </h1>
    </div>
    ...
    <div class="bg-white dark:bg-gray-800 shadow-sm rounded-lg p-6">
      ...
      >
        ...
        <FormKit
            ...
            :label="$t('sales.orders.create.shippingAddress')"
            validation="required"
        />
        ...
        <div class="mt-8">
          <h3 class="text-lg font-semibold text-gray-900 dark:text-white mb-4">{{ $t('sales.orders.create.items') }}</h3>
        ...
        <div class="flex justify-end gap-4 mt-8">
          <NuxtLink :to="`/sales/orders/${orderId}`" class="btn btn-secondary">
            {{ $t('common.cancel') }}
          </NuxtLink>
...
const statusOptions = ['Pending', 'Confirmed', 'Processing', 'Shipped', 'Completed', 'Cancelled'].map(s => ({ label: t(`sales.status.${s.toLowerCase()}`), value: s }))

onMounted(() => {
  // In a real app, you would fetch the order data from an API
  formData.value = { ...mockOrder }
  updateTotals()
...
useHead({
  title: t('sales.orders.edit.pageTitle', { id: orderId }),
})
