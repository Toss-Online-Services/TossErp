  ...
  <div class="min-h-screen bg-gray-50 dark:bg-gray-900">
      ...
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="py-4">
          <div class="flex items-center justify-between">
            <div>
              <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Leads Management</h1>
              <p class="text-gray-600 dark:text-gray-400">Track and manage potential customers</p>
            </div>
    ...
    <div v-else class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        <div class="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700">
          <div class="flex items-center justify-between">
            <div>
              ...
              <p class="text-2xl font-bold text-blue-600">{{ analytics?.leads?.totalLeads || leads.length }}</p>
            </div>
          ...
          <div class="mt-2">
            <span class="text-sm text-green-600">+{{ analytics?.leads?.newThisMonth || 0 }} this month</span>
          </div>
        </div>
        ...
        <div class="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700">
          <div class="flex items-center justify-between">
            <div>
              <p class="text-sm text-gray-600 dark:text-gray-400">Qualified Leads</p>
              <p class="text-2xl font-bold text-green-600">{{ analytics?.leads?.qualifiedLeads || 0 }}</p>
            </div>
          ...
          <div class="mt-2">
            <span class="text-sm text-gray-600 dark:text-gray-400">{{ analytics?.leads?.conversionRate || 0 }}% conversion rate</span>
          </div>
        </div>
        ...
        <div class="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700">
          <div class="flex items-center justify-between">
            <div>
              ...
              <p class="text-2xl font-bold text-yellow-600">{{ analytics?.leads?.averageScore || 0 }}</p>
            </div>
          ...
          <div class="mt-2">
            <span class="text-sm text-gray-600 dark:text-gray-400">Out of 100</span>
          </div>
        </div>
        ...
        <div class="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700">
          <div class="flex items-center justify-between">
            <div>
              ...
              <p class="text-2xl font-bold text-red-600">{{ hotLeadsCount }}</p>
            </div>
          ...
          <div class="mt-2">
            <span class="text-sm text-gray-600 dark:text-gray-400">Score > 80</span>
          </div>
        </div>
      </div>
            ...
            <div class="space-y-4">
              <div v-for="source in leadSources" :key="source.source" class="flex items-center justify-between">
                <div class="flex items-center space-x-3">
                  ...
                  <span class="text-sm font-medium text-gray-900 dark:text-white">{{ source.source }}</span>
                </div>
                <div class="flex items-center space-x-3">
                  <span class="text-sm text-gray-600 dark:text-gray-400">{{ source.count }}</span>
                  <span class="text-sm text-gray-500 dark:text-gray-500">{{ source.percentage }}%</span>
                </div>
              </div>
            </div>
          </div>
            ...
            <div class="space-y-4">
              <div v-for="status in leadStatusDistribution" :key="status.status" class="flex items-center justify-between">
                ...
                <div class="flex items-center space-x-3">
                  ...
                  <span class="text-sm text-gray-500 dark:text-gray-500">{{ status.percentage }}%</span>
                </div>
              </div>
            </div>
          </div>
      ...
      <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700">
        <div class="p-6 border-b border-gray-200 dark:border-gray-700">
          <div class="flex items-center justify-between">
            <h3 class="text-lg font-semibold text-gray-900 dark:text-white">All Leads</h3>
            <div class="flex space-x-2">
              <select v-model="statusFilter" class="text-sm border border-gray-300 dark:border-gray-600 rounded-lg px-3 py-1 bg-white dark:bg-gray-700 text-gray-900 dark:text-white">
              ...
              >
              <button @click="refreshLeads" class="bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-400 px-3 py-2 rounded-lg hover:bg-gray-200 dark:hover:bg-gray-600 transition-colors">
        ...
        <div class="overflow-x-auto">
          <table class="w-full">
            ...
            <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
              <tr v-for="lead in filteredLeads" :key="lead.id" class="hover:bg-gray-50 dark:hover:bg-gray-700">
                <td class="px-6 py-4 whitespace-nowrap">
                  <div class="flex items-center">
                    <div class="w-10 h-10 bg-blue-100 dark:bg-blue-900 rounded-full flex items-center justify-center">
                      <span class="text-blue-600 font-medium text-sm">{{ getInitials(lead.firstName, lead.lastName) }}</span>
                    </div>
                    <div class="ml-4">
                      <div class="text-sm font-medium text-gray-900 dark:text-white">{{ lead.firstName }} {{ lead.lastName }}</div>
                      <div class="text-sm text-gray-500 dark:text-gray-400">{{ lead.email }}</div>
                    </div>
                  </div>
                </td>
                <td class="px-6 py-4 whitespace-nowrap">
                  <span class="text-sm text-gray-900 dark:text-white">{{ lead.source }}</span>
                </td>
                <td class="px-6 py-4 whitespace-nowrap">
                  <div class="flex items-center">
    ...
    <div v-if="showCreateLeadModal" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div class="bg-white dark:bg-gray-800 rounded-lg p-6 w-full max-w-md">
        <h3 class="text-lg font-semibold text-gray-900 dark:text-white mb-4">Create New Lead</h3>
        <form @submit.prevent="createLead">
          <div class="space-y-4">
            <div>
              <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">First Name</label>
              <input v-model="newLead.firstName" type="text" required class="mt-1 block w-full border border-gray-300 dark:border-gray-600 rounded-lg px-3 py-2 bg-white dark:bg-gray-700 text-gray-900 dark:text-white">
            </div>
            <div>
              <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">Last Name</label>
              <input v-model="newLead.lastName" type="text" required class="mt-1 block w-full border border-gray-300 dark:border-gray-600 rounded-lg px-3 py-2 bg-white dark:bg-gray-700 text-gray-900 dark:text-white">
            </div>
            <div>
              <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">Email</label>
              <input v-model="newLead.email" type="email" required class="mt-1 block w-full border border-gray-300 dark:border-gray-600 rounded-lg px-3 py-2 bg-white dark:bg-gray-700 text-gray-900 dark:text-white">
            </div>
            <div>
              <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">Phone</label>
              <input v-model="newLead.phone" type="tel" required class="mt-1 block w-full border border-gray-300 dark:border-gray-600 rounded-lg px-3 py-2 bg-white dark:bg-gray-700 text-gray-900 dark:text-white">
            </div>
            <div>
              <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">Company</label>
              <input v-model="newLead.company" type="text" class="mt-1 block w-full border border-gray-300 dark:border-gray-600 rounded-lg px-3 py-2 bg-white dark:bg-gray-700 text-gray-900 dark:text-white">
            </div>
            <div>
              <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">Source</label>
              <select v-model="newLead.source" required class="mt-1 block w-full border border-gray-300 dark:border-gray-600 rounded-lg px-3 py-2 bg-white dark:bg-gray-700 text-gray-900 dark:text-white">
          ...
          <div class="flex justify-end space-x-3 mt-6">
            <button type="button" @click="showCreateLeadModal = false" class="px-4 py-2 border border-gray-300 rounded-lg text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700">
              Cancel
            </button>
