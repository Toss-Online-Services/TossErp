internal static class Extensions
{
    public static void AddApplicationServices(this IHostApplicationBuilder builder)
    {
        ...
        // The DbContext of type 'POSContext' cannot be pooled because it does not have a public constructor accepting a single parameter of type DbContextOptions or has more than one constructor.
        services.AddDbContext<POSContext>(options =>
        {
            options.UseNpgsql(builder.Configuration.GetConnectionString("POSdb"));
        });
        builder.EnrichNpgsqlDbContext<POSContext>();

        services.AddMigration<POSContext, POSContextSeed>();

        // Add the integration services that consume the DbContext
        services.AddTransient<IIntegrationEventLogService, IntegrationEventLogService<POSContext>>();

        services.AddTransient<IPOSIntegrationEventService, POSIntegrationEventService>();
