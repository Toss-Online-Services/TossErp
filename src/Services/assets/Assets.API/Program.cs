...
Log.Logger = new LoggerConfiguration()
    ...
    .Enrich.WithProperty("ApplicationName", "Assets.API")
    .CreateLogger();
...
builder.Services.AddSwaggerGen(c =>
{
    c.SwaggerDoc("v1", new OpenApiInfo
    {
        Title = "Assets API",
        Version = "v1",
        Description = "TOSS ERP Assets Management Service API - Multi-tenant asset tracking and management",
...
builder.Services.AddAuthorization(options =>
{
    ...
    options.AddPolicy("AssetManager", policy => policy.RequireRole("Admin", "AssetManager"));
    options.AddPolicy("AssetUser", policy => policy.RequireRole("Admin", "AssetManager", "AssetUser"));
});
...
builder.Services.AddHealthChecks()
    ...
    .AddSqlServer(
        ...
        name: "assets-db",
        tags: new[] { "ready", "db" })
    .AddCheck<AssetsServiceHealthCheck>("assets-service", tags: new[] { "ready", "service" });
...
{
    Log.Information("Assets API is shutting down...");
});

lifetime.ApplicationStopped.Register(() =>
{
    Log.Information("Assets API has stopped");
    Log.CloseAndFlush();
});

try
{
    Log.Information("Starting Assets API...");
    app.Run();
}
catch (Exception ex)
{
    Log.Fatal(ex, "Assets API failed to start");
    throw;
}
finally
{
...
public class AssetsServiceHealthCheck : IHealthCheck
{
    private readonly ILogger<AssetsServiceHealthCheck> _logger;

    public AssetsServiceHealthCheck(ILogger<AssetsServiceHealthCheck> logger)
    {
        _logger = logger;
    }
    ...
    public async Task<HealthCheckResult> CheckHealthAsync(HealthCheckContext context, CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            return HealthCheckResult.Healthy("Assets service is healthy");
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Health check failed");
            return HealthCheckResult.Unhealthy("Assets service is unhealthy", ex);
        }
    }
}
